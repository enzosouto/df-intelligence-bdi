...
async def lifespan(app: FastAPI):
    # Abrir o pool já na subida é só adiantamento: `db.cursor()` abre sob
    # demanda. Se o banco não responder agora (Neon acordando, função
    # serverless fria), a API sobe mesmo assim e o health-check responde — em
    # vez de derrubar todas as rotas por causa da primeira conexão.
    try:
        db.init_pool()
    except Exception as exc:  # noqa: BLE001
        logger.warning("Pool não aberto na subida (%s); será aberto na primeira consulta.", exc)
    yield
    db.close_pool()
